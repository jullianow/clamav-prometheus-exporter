...
This project follows the [Conventional Commits](https://www.conventionalcommits.org/) specification for commit messages. This ensures consistent commit history and enables automated changelog generation.
...
```
feat: add new ClamAV scan metrics
fix(pkg/collector): resolve memory leak in metrics collection
docs: update README with installation instructions
refactor(pkg/clamav): simplify client connection logic
test: add unit tests for collector package
```
...
```
feat!: change metrics endpoint from /metrics to /prometheus

BREAKING CHANGE: The metrics endpoint has been moved from /metrics to /prometheus
```

### Changelog Generation

This project uses [git-cliff](https://github.com/orhun/git-cliff) to automatically generate changelogs from conventional commit messages.
...
# Ubuntu/Debian: apt install git-cliff
...
The git-cliff configuration is stored in `cliff.toml` and includes:
- Commit message parsing rules
- Changelog formatting templates
